...
export default function TrainList(props) {
    ...
    console.log(filter)
    const filtered_arrivals = arrivals.filter((arrival) => arrival.LINE === color && f(arrival.STATION, station) && ff(arrival, filter))
    return (
        <div>
            {filtered_arrivals.length > 0 ? filtered_arrivals.map(
                            (arrival) => {
